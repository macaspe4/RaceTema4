...
save(file = './data/2024-03-12.Rdata',list=c('mydf'), eval = FALSE)
...
#convertimos los datos y guardamos el resultado en cran
cran <- as_tibble(mydf)
...
```{r}
#viaulizamos directamente cran
cran

#visualizamos con glimpse
glimpse(cran)
...
```{r}
#seleccionamos solo ip_id, package, y country
select(cran, ip_id, package, country)

```
5. Elige todas las variables entre `r_arch y country` 
```{r}
#seleccionamos todas r-arch y country
...
```{r}
#seleccionamos todos menos date y size
...
```{r}
#seleccionamos aquellos que tengo una version 3.5.1
cran %>% 
...
```{r}
#seleccionamos aquellas versiones mayores a 3.4.0
cran %>%
...
```{r}
#seleccionamos aquellas realizadas desde España o Portugal
cran %>%
...
```{r}
cran %>%
  dplyr::filter(r_os == "linux-gnu" & size > "100500")
...
```{r}
umbral <- quantile(pack_sum$count, probs = 0.99, na.rm = T)

#top_counts <- by_package %>%
  #filter(pack_sum$count > umbral)
